...
// AI問診（9問診断・自由入力の症状分析）の有効/無効スイッチ
//   false : AI問診を停止。キャンセルフローのみAIが自動返信する。
//           問診関連のメッセージには自動返信せず、記録のみ行う。
//   true  : AI問診を有効化（従来どおり）。
// ※ 問診を再開したい場合は true に戻すだけでOKです。
const AI_INQUIRY_ENABLED = false;
...
async function handleEvent(event) {
  ...
  // ※ AI_INQUIRY_ENABLED が false の間は問診を開始しない（記録のみ）。
  if (AI_INQUIRY_ENABLED &&
      (userText === RICH_MENU_ACTIONS.AI_INQUIRY || userText.includes('AI問診') || userText.includes('AI診断'))) {
    await setModeSafe(userId, 'ai');
  ...
  // ※ AI_INQUIRY_ENABLED が false の間は進行中の問診も処理しない。
  if (AI_INQUIRY_ENABLED && inquiryStep >= 1) {
    return handleInquiryStep(event.replyToken, userId, displayName, userText, inquiryStep);
  }

  // ---- ⑩ AI問診が無効の場合 → 自動返信せず記録のみ ----------------------
  // キャンセルフロー（①）以外の自由入力にはAIが反応しないようにする。
  if (!AI_INQUIRY_ENABLED) {
    await logHistory({ userId, displayName, eventType: '自由入力（AI問診停止中）', content: userText });
    await safe('最終時刻更新', () => updateUserData(userId, { lastMessageAt: new Date().toISOString() }));
    return null;
  }
